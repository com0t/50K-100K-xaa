...
class WDW_FM_Library {
  ...
  public static function get_submissions_to_export() {
    ...
    $query_lable = $wpdb->prepare("SELECT label_order,title,mail_verify,send_to FROM " . $wpdb->prefix . "formmaker where id=%d", $form_id);
    ...
    if ( !empty($groupids) ) {
      ...
      for ( $www = 0; $www < count($groupids); $www++ ) {
        ...
        if ( $label_type[$field_key] != 'type_arithmetic_captcha' && $label_type[$field_key] != 'type_stripe' ) {
          ...
          if ( !empty($rows_lable[0]->mail_verify) && !empty($rows_lable[0]->send_to) ) {
            $send_mail_ids = explode('**', $rows_lable[0]->send_to);
              ...
              foreach ( $send_mail_ids as $send_mail_id ) {
                ...
                foreach ( $element_values as $v_index => $v_value ) {
                  if ( $v_value == 'verified**' . $send_mail_id ) {
                    ...
                    var_dump($label_id_by_send_mail_id, $element_values[$label_id_by_send_mail_id]);
